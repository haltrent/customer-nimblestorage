...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="@type='caution'">
                        <fo:inline xsl:use-attribute-sets="note__label__caution">
                            <xsl:call-template name="getVariable">
                                <xsl:with-param name="id" select="'Nimble Caution'"/>
                            </xsl:call-template>
                        </fo:inline>
                    </xsl:when>
        ...
        <xsl:choose>
            ...
            <xsl:when test="@otherprops">
                ...
                <fo:inline xsl:use-attribute-sets="ph">
                    <xsl:call-template name="commonattributes"/>
                    <xsl:call-template name="getVariable">
                        <xsl:with-param name="id" select="concat($BRAND,' ',normalize-space(@otherprops))"/>
                    </xsl:call-template>
                </fo:inline>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="*" mode="placeImageTEST">
        ...
        <fo:external-graphic src="url({$href})" content-width="scale-to-fit" content-height="100%" scaling="uniform" max-width="6.5in"/>
